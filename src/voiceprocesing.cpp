...
/* 4 QComboBox::clear() + 20
1   freeroad.ch                   	0x000000010fc4b669 VoiceProcesing::setVoices() + 1481 (voiceprocesing.cpp:177)
2   freeroad.ch                   	0x000000010fc4a0fb VoiceProcesing::VoiceProcesing(QWidget*) + 1131 (voiceprocesing.cpp:68)
3   freeroad.ch                   	0x000000010fc49c7d VoiceProcesing::VoiceProcesing(QWidget*) + 29 (voiceprocesing.c */
...
VoiceProcesing::VoiceProcesing(QWidget *parent) : QDialog(parent) {
  ...
  QObject::connect(this, SIGNAL(setValuePros(int)), progreso,SLOT(setValue(int)));
  QObject::connect(voicenames, SIGNAL(activated(int)), this, SLOT(setVoiceatd(int)));
  QObject::connect(voiceformat, SIGNAL(activated(int)), this, SLOT(setFormatdAudioOut(int)));
}
...
void VoiceProcesing::setVoices() {
    ...
    voiceformat->clear();
    QString aa;
    aa = QString("m4bf  Apple MPEG-4 AudioBooks / (.m4b) [aac,aace,aacf,aach,aacl,aacp]");
    voiceformat->addItem(aa,QVariant("m4b"));

    aa = QString("m4af  Apple MPEG-4 Audio / (.m4a,.m4r) [aac,aace,aacf,aach,aacl,aacp,alac]");
    ...
    aa = QString("3gp2  3GPP-2 Audio / (.3gp) [Qclp,aac,aace,aacf,aach,aacl,aacp]");
    voiceformat->addItem(aa,QVariant("3gp"));

    aa = QString("AIFC  AIFC Audio / (.aifc,.aiff,.aif) [lpcm,ulaw,alaw,ima4,Qclp]");
    ...
    aa = QString("caff  CAF Audio / (.caf) [Qclp,aac,aace,aacf,aach,aacl,aacp,alac,alaw,ilbc,ima4,lpcm,ulaw]");
...
void VoiceProcesing::speechEnd() {
    ...
    QString outmove = QString("%1/rec_%2_%3_audio.").arg(musicdir).arg(Voicecurrent.voicename).arg(sec1970);
