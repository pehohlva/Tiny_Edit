...
QString DOC::GetHtml( const QString file ) {
    ...
    QString logx =  QString("{%1} - ").arg(MIMENAME) + QString(alii.join(",")) + QString(" - ") + namef;
...
void DOC::wakeUpContenent( QString plaintext , const QFileInfo file ) {
    ...
    if (totalwords < 2) {
     ...
     if (file.exists()) {
    QString rec = DOC_INFONOW + QString(" Word:%1/Char:%2").arg(totalwords).arg(bhsize) + QString("-Lang:%1").arg(lang);
    emit setMessage(rec);
     } else {
         if (bhsize < 33) {
         QString rec = _NONAMEFILE_ + QString(" / ") + QString("-Lang:%1").arg(lang);
         emit setMessage(rec);
         } else {
            QString rec = _NONAMEFILE_ + QString(" / Save your work..") + QString("-Lang:%1").arg(lang);
            emit setMessage(rec);
         }
     }
...
void DOC::log_documents( QString xtcr ) {
    ...
    QString logdd = readcfile(_debuglogfile_);
     ...
     QFile f(_debuglogfile_);
               if ( f.open( QFile::Truncate | QFile::Text ) ) {
                             QTextStream sw( &f );
